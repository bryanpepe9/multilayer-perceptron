...
func LoadPatternsFromCSVFile(filePath string) ([]Pattern, error, []string) {
	...
	fileContent, error := os.ReadFile(filePath)

	if error != nil {
		// NOTE: this must not use log.Fatal - that calls os.Exit and would
		// kill the whole process, even though this function already reports
		// the failure to its caller via the returned error.
		log.WithFields(log.Fields{
			...
			"filePath": filePath,
			"error":    error,
		}).Error("Failed to read file in specified path.")
