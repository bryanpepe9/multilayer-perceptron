...
// RandomNeuronInit initializes a neuron's weights and bias with small random
// values, using Xavier/Glorot-style scaling (stddev = 1/sqrt(fan-in)).
//
// The scale matters: initializing weights too close to zero (as a previous
// version of this function did, using a 1e-13 scaling factor) makes every
// neuron in a layer start out virtually identical. Since backpropagation
// then nudges them by nearly the same amount on every step, they never
// differentiate from one another - which silently caps the network's
// capacity to below what a single neuron could do, making problems that
// require a genuine hidden layer (e.g. XOR) impossible to learn no matter
...
func RandomNeuronInit(neuron *NeuronUnit, dim int) {
	...
	for index := range neuron.Weights {
		...
		neuron.Weights[index] = rand.NormFloat64() * scale
	}

	// init random bias; learning rate/value/delta are working state that
	// gets overwritten before use, but are seeded here too for consistency.
	neuron.Bias = rand.NormFloat64() * scale
	neuron.Lrate = rand.NormFloat64() * scale
	neuron.Value = rand.NormFloat64() * scale
	neuron.Delta = rand.NormFloat64() * scale
